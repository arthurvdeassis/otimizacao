...
# Função para plotar as cidades a partir de um arquivo
...
class AntColonyOptimization:
    def __init__(self, num_cities, distance_matrix, num_ants, num_iterations, alpha, beta, evaporation_rate, pheromone_constant, initial_pheromone_level, pheromone_min, pheromone_max, chromosome_size, mutation_rate, elitism_rate, crossover_rate):
        ...
        self.pheromone_levels = np.ones((num_cities, num_cities)) * initial_pheromone_level
    ...
    def select_next_city(self, current_city, unvisited_cities):
        pheromones = np.array([self.pheromone_levels[current_city, next_city] for next_city in unvisited_cities])
        distances = np.array([self.distance_matrix[current_city, next_city] for next_city in unvisited_cities])
        desirability = pheromones ** self.alpha * (1.0 / distances) ** self.beta
    ...
    def update_pheromones(self, routes, route_lengths):
        ...
        for route, route_length in zip(routes, route_lengths):
            ...
            for i in range(len(route) - 1):
                updated_pheromone = self.pheromone_levels[route[i], route[i + 1]] + pheromone_delta
                self.pheromone_levels[route[i], route[i + 1]] = np.clip(updated_pheromone, self.pheromone_min, self.pheromone_max)
                self.pheromone_levels[route[i + 1], route[i]] = np.clip(updated_pheromone, self.pheromone_min, self.pheromone_max)
...
def run_experiment(num_ants, num_iterations, alpha, beta, evaporation_rate, pheromone_constant, chromosome_size, mutation_rate, elitism_rate, crossover_rate, num_runs):
    try:
        cities = pd.read_csv("cidades.txt", sep=r'\s+', header=None, names=["x", "y"])
    ...
    for run in range(num_runs):
        aco = AntColonyOptimization(num_cities, distance_matrix, num_ants, num_iterations, alpha, beta, evaporation_rate, pheromone_constant, initial_pheromone_level=0.1, pheromone_min=0.1, pheromone_max=10.0, chromosome_size=chromosome_size, mutation_rate=mutation_rate, elitism_rate=elitism_rate, crossover_rate=crossover_rate)
    ...
    plt.plot(range(1, num_iterations + 1), mean_fitness, marker="o", linestyle="-", color="black", label="FITNESS")
...
if __name__ == "__main__":
    plot_cities('cidades.txt')  # Plot inicial das cidades
